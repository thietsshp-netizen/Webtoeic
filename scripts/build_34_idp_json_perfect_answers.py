...
answer_keys = extract_all_clean_answers()
print(f"Extracted Perfect Answer Keys for all {len(answer_keys)} tests.")

# Map EXACT top-header page boundaries
test_page_starts = {}
...
print(f"Mapped EXACT top-header page boundaries for {len(test_starts)} tests.")
...
def process_idp_passage_dynamic(test_idx, p_num, sec_text, valid_q_min, valid_q_max):
    ...
    # STOP passage text at the FIRST question header
    q_start_pos = re.search(r'\n\s*Questions?\s+\d+', sec_text, re.IGNORECASE)
    passage_body_text = sec_text[:q_start_pos.start()] if q_start_pos else sec_text

    sec_clean = re.sub(r'©\s*The British Council.*?\d+', '', passage_body_text)
    ...
    # Extract Question Groups FROM FULL SEC_TEXT
    q_blocks = re.split(r'(?:^|\n|\.\s+|\s{2,})(Questions?\s+\d+\s*[\–\-\—\s]?\s*\d*)', sec_text, flags=re.IGNORECASE)
    question_groups = []
    group_idx = 1
    ...
    if not question_groups:
        ...
        for q_idx in range(valid_q_min, valid_q_max + 1):
            ...
            ev_sid = f"p{p_num}-s1"
            ev_text_vi = translation_map.get(ev_sid, "")
            explanation_obj = format_explanation(q_idx, "SHORT_ANSWER", f"Question {q_idx}", ans, ev_sid, ev_text_vi, f"Câu hỏi {q_idx}")
...
# Process all 34 Reading Tests STRICTLY BY EXACT TOP HEADERS
print("\nStarting PERFECT DYNAMIC & SEQUENTIAL generation of all 102 IDP JSON files...")
...
print("\nFinished generating all 102 IDP JSON files PERFECTLY!")
