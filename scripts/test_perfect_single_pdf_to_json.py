...
def parse_answers_from_ak_pages(ak_text):
...
def process_single_split_pdf(test_id, pdf_path):
    reader = pypdf.PdfReader(pdf_path)
    n_pages = len(reader.pages)
    
    # Extract Answer Key from last page(s)
    # If test has multi-page AK (like test 6), AK starts at first page with 'Section 1' or '1 '
    ak_start = n_pages - 1
    ...
    test_answers = parse_answers_from_ak_pages(ak_txt)
    print(f"\nProcessing Test {test_id} from {pdf_path}: {len(test_answers)} answer keys parsed.")

    # Split into 3 Passage Sections
    ...
    for p_num, sec_text in sections:
        # STRICT SEPARATION: Passage Text is BEFORE the first 'Questions X'
        q_start_pos = re.search(r'\n\s*Questions?\s+\d+', sec_text, re.IGNORECASE)
        if q_start_pos:
            passage_raw = sec_text[:q_start_pos.start()]
            questions_raw = sec_text[q_start_pos.start():]
        else:
        ...
        # Check for paragraph labels A., B., C.
        ...
        # Detect dynamic question range for this passage
        q_hdrs = re.findall(r'Questions?\s+(\d+)\s*[\–\-\—\s]*(\d*)', questions_raw, re.IGNORECASE)
        q_nums = []
        for q1, q2 in q_hdrs:
            if q1: q_nums.append(int(q1))
            if q2: q_nums.append(int(q2))
        
        valid_q_min = min(q_nums) if q_nums else (1 if p_num==1 else (14 if p_num==2 else 27))
        valid_q_max = max(q_nums) if q_nums else (13 if p_num==1 else (26 if p_num==2 else 40))
...
# Test on test 1 and test 7
process_single_split_pdf(1, '/Volumes/MacOS Sandisk/Users/thietphamvan/hoctoeic/IELTS/34_DE_THI_IDP_SPLIT/idp_test_01.pdf')
process_single_split_pdf(7, '/Volumes/MacOS Sandisk/Users/thietphamvan/hoctoeic/IELTS/34_DE_THI_IDP_SPLIT/idp_test_07.pdf')
