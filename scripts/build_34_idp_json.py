...
print(f"Extracted Answer Keys for {len(answer_keys)} tests from PDF pages 460-{len(reader.pages)}.")

# Build page ranges for all 34 Reading Tests
test_starts = []
for p_idx in range(0, 458):
    txt = reader.pages[p_idx].extract_text()
    m = re.search(r'\bReading\s+Test\s+(\d+)\b', txt, re.IGNORECASE)
    if m:
        t_num = int(m.group(1))
        # Keep first occurrence after TOC page (page index >= 3)
        if p_idx >= 3 and not any(t == t_num for t, _ in test_starts):
            test_starts.append((t_num, p_idx))

test_starts.sort(key=lambda x: x[1])
print(f"Identified test section boundaries across PDF pages for {len(test_starts)} tests.")

def parse_q_block_idp(header, raw_block, test_answers, sid_map, translation_map, p_num, group_idx):
...
def process_idp_passage(test_idx, p_num, sec_text):
    ...
    # Extract Question Groups from sec_clean
    ...
    for i in range(1, len(q_blocks), 2):
        ...
        g_obj = parse_q_block_idp(header, block_text, test_answers, sid_map, translation_map, p_num, group_idx)
        if g_obj:
    ...
    print(f"Generated {out_filename}: {title[:35]} ({len(translation_map)} sents, {len(question_groups)} q_groups)")

# Process all 34 Reading Tests
print("\nStarting generation of all 102 IDP JSON files...")
for idx, (t_idx, start_page) in enumerate(test_starts):
    end_page = test_starts[idx+1][1] if idx + 1 < len(test_starts) else 458
    print(f"\n--- Test {t_idx} (Pages {start_page+1}-{end_page}) ---")
    ...
    process_idp_passage(t_idx, 1, sec1_txt)
    process_idp_passage(t_idx, 2, sec2_txt)
    process_idp_passage(t_idx, 3, sec3_txt)

print("\nFinished generating all 102 IDP JSON files successfully!")
