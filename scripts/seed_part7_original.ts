...
async function main() {
  console.log("🚀 STARTING ORIGINAL SEED: Seeding Part 7 with 100% original data...");
  ...
  for (const row of data) {
    try {
      ...
      count++;
      if (count % 100 === 0) console.log(`Restored ${count} groups...`);

    } catch (err) {
      console.error(`Row ${row.QuestionRange} failed:`, err);
    }
  }

  console.log(`\n✅ RESTORATION COMPLETE! Re-seeded ${count} Part 7 groups with 100% original data.`);
}
