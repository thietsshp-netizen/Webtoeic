                          ...
                          <div>
                            ...
                            <h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-full bg-slate-100 text-slate-400 hover:bg-blue-50 hover:text-blue-500 transition-all"
                                title="Hướng dẫn học"
                              >
                                <HelpCircle size={18} />
                              </button>
      ...
      >
        ...
        <div className="absolute inset-0 bg-white rounded-[2.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border-2 border-slate-100 p-8 flex flex-col backface-hidden group-hover/card:shadow-xl transition-all">
          <button
            ...
            className={`absolute top-3 left-3 p-1.5 rounded-lg transition-all ${vocab.isUnlearned ? "text-rose-500 bg-rose-50 scale-105 shadow-sm ring-1 ring-rose-100" : "text-slate-200 hover:text-rose-400 hover:bg-slate-50"}`}
          >
          ...
          <button
            ...
            className={`absolute top-3 right-3 p-1.5 rounded-lg transition-all text-amber-400 bg-amber-50 scale-105 shadow-sm ring-1 ring-amber-100`}
          >
...
// --- MODAL HƯỚNG DẪN HỌC ---
function VocabGuideModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
...
function VocabGuideContent({ onClose }: { onClose?: () => void }) {
  return (
    <div className="space-y-12">
      {/* Intro */}
      <section className="space-y-4">
        <p className="text-slate-600 font-medium leading-relaxed">
          Chào mừng bạn đến với hệ thống học từ vựng thông minh của WebTOEIC. Hệ thống này không chỉ là nơi lưu trữ, mà còn là một "huấn luyện viên" giúp bạn ghi nhớ từ vựng vào trí nhớ dài hạn bằng phương pháp <strong className="text-blue-600">Lặp lại ngắt quãng (Spaced Repetition System - SRS)</strong> dựa trên quy tắc 5 Hộp Leitner.
        </p>
      </section>
