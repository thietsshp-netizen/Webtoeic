...
import { ChevronLeft, ChevronRight, MessageSquareQuote, ZoomIn, X, ShieldCheck } from "lucide-react";
...
interface StudentFeedbackShowcaseProps {
  images: FeedbackImage[];
}

export default function StudentFeedbackShowcase({ images }: StudentFeedbackShowcaseProps) {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="w-full aspect-[1.5/1] sm:aspect-[1.8/1] md:aspect-[1.95/1] bg-slate-100 animate-pulse rounded-2xl sm:rounded-3xl flex items-center justify-center">
        <div className="flex items-center gap-2 text-slate-400 text-sm font-semibold">
          <MessageSquareQuote size={24} className="animate-pulse text-blue-500" />
          <span>Đang tải cảm nhận học viên...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-4 select-none">
      ...
      <div
        className="relative group max-w-xl sm:max-w-2xl mx-auto rounded-[1.8rem] sm:rounded-[2.2rem] overflow-hidden border-2 border-amber-400 bg-white p-1 sm:p-1.5 shadow-sm cursor-zoom-in"
      ...
      >
        <div className="relative w-full aspect-[1.176/1] bg-white rounded-[1.4rem] sm:rounded-[1.8rem] flex items-center justify-center overflow-hidden">
          {images.map((img, idx) => {
                ...
                {!isLoaded && (
                  <div className="absolute inset-2 bg-slate-200 animate-pulse rounded-2xl flex items-center justify-center">
                    <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold">
                      <MessageSquareQuote size={20} className="animate-pulse text-blue-500" />
                      <span>Đang tải cảm nhận {idx + 1}...</span>
                    </div>
                  </div>
                )}
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <h3 className="text-sm sm:text-base font-bold text-white">Cảm Nhận Học Viên ({activeIdx + 1}/{total})</h3>
                <p className="text-[10px] sm:text-xs text-slate-400">Chia sẻ chân thực từ học viên hoctoeic.com</p>
              </div>
            </div>
          ...
          >
            <img
              src={currentImg.url}
              alt={`Cảm nhận học viên phóng to ${activeIdx + 1}`}
            ...
            {total > 1 && (
              <>
                <button
                  ...
                  title="Xem cảm nhận trước (Mũi tên trái)"
                >
                  <ChevronLeft size={20} />
                </button>
                <button
                  ...
                  title="Xem cảm nhận tiếp theo (Mũi tên phải)"
                >
                  <ChevronRight size={20} />
                </button>
              </>
