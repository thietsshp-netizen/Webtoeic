...
interface Part7SmartSelectorProps {
  onSelect: (filters: { day?: string; type?: string; book?: string; all?: boolean; testFull?: string }) => void;
  initialFilters?: { day?: string; type?: string; book?: string; all?: boolean; testFull?: string };
}

export default function Part7SmartSelector({ onSelect, initialFilters }: Part7SmartSelectorProps) {
  ...
  useEffect(() => {
    async function fetchFilters() {
      try {
        const res = await fetch("/api/admin/part7/filters");
        ...
        console.error("Lỗi nạp bộ lọc Part 7:", err);
      } finally {
  ...
  useEffect(() => {
    ...
    async function fetchPreview() {
      ...
      try {
        ...
        if (mode === "test") params.append("testFull", value);

        const res = await fetch(`/api/admin/part7/selection?${params.toString()}`);
        const data = await res.json();
        if (data.success) {
          ...
          else if (mode === "test") filterObj.testFull = value;
          
          onSelect(filterObj);
        }
      } catch (err) {
        console.error("Lỗi nạp preview Part 7:", err);
      } finally {
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
          <Filter size={12} className="text-orange-500" /> Cấu hình Ngân hàng Part 7 động
        </h3>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
               ...
               className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${selectedValue === "all:true" ? "bg-orange-600 text-white border-orange-600 shadow-lg shadow-orange-200" : "bg-white text-slate-600 border-slate-100 hover:border-orange-200 hover:bg-orange-50/50"}`}
            >
              🚀 Toàn bộ Part 7
            </button>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">Hoặc chọn chi tiết (theo Day, Sách)</label>
          <select
            value={selectedValue}
            onChange={(e) => setSelectedValue(e.target.value)}
            className="w-full p-4 rounded-2xl border border-slate-100 bg-slate-50 outline-none focus:ring-2 focus:ring-orange-500 font-black text-slate-700 transition-all cursor-pointer text-sm"
          >
            <option value="">-- Vui lòng chọn một nhóm để nạp --</option>
            <option value="all:true">Toàn bộ ngân hàng Part 7</option>
            
            {tests.length > 0 && (
              <optgroup label="📜 PHÂN LOẠI THEO ĐỀ (BOOK-TEST)">
                {tests.map(test => (
                  <option key={`test:${test}`} value={`test:${test}`}>Đề: {test}</option>
                ))}
              </optgroup>
            )}

            {books.length > 0 && (
              <optgroup label="📚 PHÂN LOẠI THEO BỘ SÁCH (BOOK SERIES)">
                {books.map(book => (
                  <option key={`book:${book}`} value={`book:${book}`}>Dòng sách: {book}</option>
                ))}
              </optgroup>
            )}
          ...
          <div className="pt-4 border-t border-slate-50">
             <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <List size={12} /> Số nhóm câu hỏi sẽ nạp: {previewGroups.length}
                </span>
                {loadingPreview && <Loader2 size={14} className="animate-spin text-orange-500" />}
             </div>

             <div className="max-h-60 overflow-y-auto space-y-3 pr-2 scrollbar-thin">
                {previewGroups.map((g, i) => (
                  <div key={g.id} className="p-4 bg-slate-50 hover:bg-orange-50/30 transition-colors rounded-2xl border border-slate-100 flex items-start gap-4">
                     <div className="p-3 bg-white rounded-xl shadow-sm border border-slate-100">
                       <FileText size={20} className="text-slate-400" />
                     </div>
                     <div className="flex-1 space-y-1 mt-1 text-left">
                       <div className="flex items-center gap-2">
                          <span className="bg-orange-100 text-orange-700 font-bold text-[9px] uppercase px-2 py-0.5 rounded-md">
                            Nhóm {i + 1}
                          </span>
