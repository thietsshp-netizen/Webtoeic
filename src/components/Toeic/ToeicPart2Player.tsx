...
export default function ToeicPart2Player({
  ...
  const [wordFamiliesData, setWordFamiliesData] = useState<any[]>([]);
  ...
  useEffect(() => {
    fetch("/api/word-families?part=2")
      .then((res) => res.json())
      .then((res) => {
  ...
  const getMatchedFamiliesForQuestion = (index: number) => {
    ...
    if (!group) return [];
    const qData = group.questions?.[0] || {};
    
    let richData: any = null;
    try {
      const meta = qData.metadata as any;
      if (meta?.vietnamese && typeof meta.vietnamese === 'object') {
        const v = meta.vietnamese;
        richData = {
          question: { en: v.question_en || "", vi: v.question },
        ...
        const parsed = typeof qData.explanation === 'string' ? JSON.parse(qData.explanation) : qData.explanation;
    ...
    const engP = parseOptionsFromText(group.transcript || "");
    const questionText = richData?.question?.en || engP.find((p: any) => p.label === '?' || p.label === 'Q')?.text || qData.questionText || "";
    ...
    const matchedFams: any[] = [];

    wordFamiliesData.forEach((fam) => {
      if (!fam.words) return;
      
      const found = fam.words.some((member: string) => {
        const mLower = member.toLowerCase();
        const escaped = mLower.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        const isPhrase = mLower.includes(' ');
        
        let regex: RegExp;
        if (isPhrase) {
          regex = new RegExp(`\\b${escaped}\\b`, 'i');
        } else {
          regex = new RegExp(`\\b${escaped}(?:ing|ed|s|es|er|est|ly|y)?\\b`, 'i');
        }
        return regex.test(cleanText);
      });

      if (found) {
        matchedFams.push({ ...fam, matchedWord: fam.key });
      }
    });
  ...
  const updateCloudPopup = async (index: number, targetCloudIndex: number) => {
    ...
    if (!group) return;
    const qData = group.questions?.[0] || {};
    const correctAnswer = qData.correctAnswer?.toUpperCase();
    ...
    const questionText = richData?.question?.en || engP.find((p: any) => p.label === '?' || p.label === 'Q')?.text || qData.questionText || "";
    const questionTextVi = richData?.question?.vi || (typeof richData?.question === 'object' ? richData.question.vi : null) || explanationObj.vietText || vieParts.find((p: any) => p.label === 'Q' || p.label === '?')?.text || "";
    const questionViSpan = questionTextVi ? ` <span style="color: #94a3b8; font-weight: normal; font-size: 13px;">(${escapeHtml(questionTextVi)})</span>` : '';
    ...
    if (matchedFamilies.length > 0) {
      ...
      fam = matchedFamilies[activeIdx];
      setSelectedCloudIndex(activeIdx);
      paginationText = `${activeIdx + 1} / ${matchedFamilies.length}`;
    } else {
      setSelectedCloudIndex(-1);
    }

 
    const optionHtmls = ['A', 'B', 'C'].map(opt => {
      ...
      let suggestedHtml = '';
      if (incorrectRationale?.suggested_question) {
        const contextIntro = incorrectRationale.context_intro || "Câu này sẽ ĐÚNG nếu câu hỏi là:";
        const sugEn = typeof incorrectRationale.suggested_question === 'object' ? incorrectRationale.suggested_question.en : incorrectRationale.suggested_question;
        const sugVi = typeof incorrectRationale.suggested_question === 'object' ? incorrectRationale.suggested_question.vi : '';
        suggestedHtml = `
          <div style="margin-top: 5px; margin-left: 18px; margin-bottom: 8px; padding: 6px 10px; background-color: #fef2f2; border-left: 3px solid #ef4444; border-radius: 6px; font-size: 12px; line-height: 1.45;">
            <div style="color: #b91c1c; font-weight: 700; font-size: 10.5px; margin-bottom: 2px;">${escapeHtml(contextIntro)}</div>
            <div style="color: #0f172a; font-weight: bold;">• ${escapeHtml(sugEn || '')}</div>
            ${sugVi ? `<div style="color: #7f1d1d; opacity: 0.8; font-style: italic; margin-top: 2px;">→ ${escapeHtml(sugVi)}</div>` : ''}
          </div>
        `;
      }
    ...
    if (!popup || popup.closed) {
      popup = window.open("", "vocab_cloud", `width=${width},height=${height}`);
    ...
    if (popup) {
      ...
      popup.document.write(`
        <html>
          <head>
            <title>Gợi ý & Đám mây từ vựng</title>
            <style>
              body {
                font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
                padding: 16px;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (target && target.id === 'flag-note-textarea') return;
      ...
      if (key === '`') {
        ...
        latestPlaySegmentRef.current(labels[parseInt(key) - 1]);
        return;
  ...
  }, [currentIndex, data.length, isFullTest, onPrevPart, onNextPart, isAdminMode, canEdit, selectedCloudIndex, wordFamiliesData, revealEnglishMode]);
  ...
  return (
    <div className="absolute inset-0 flex flex-col font-sans bg-[#f8fafc] text-slate-800 overflow-hidden pr-0 lg:pr-20">
                ...
                <div className={`border p-3.5 rounded-2xl mb-1 transition-all overflow-visible ${mode === 'dictation' ? 'border-pink-200 bg-pink-50/30' : 'border-blue-100 bg-blue-50/30'}`}>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 text-lg font-bold text-slate-800 leading-relaxed min-h-[40px]">
                      ...
                      ) : (revealMode || isHintMode || revealEnglishMode) ? (
        ...
        {!isFullTest && mounted && createPortal(
          <>
            ...
            <div
              ...
              ${isSidebarHovered ? "w-72 sm:w-80 bg-slate-900/95 backdrop-blur-xl flex" : "w-[clamp(18px,2.2vw,32px)] bg-[#fbfcfd] border-l border-slate-100 cursor-pointer hidden lg:flex"}
            `}
                  ...
                  <div className="flex flex-col items-center justify-between h-full py-1 select-none">
                    ...
                    <div className="flex flex-col items-center gap-1 my-auto">
                      <div className="text-[clamp(6px,0.9vh,9px)] font-black text-blue-600">
                        {Math.round((Object.keys(answers).length / data.length) * 100)}%
                      </div>
      ...
      {(() => {
        const navContent = (
          <div id="toeic-navigation-container" className="flex items-center bg-white rounded-full p-1 sm:p-1.5 border border-slate-200/60 shadow-[0_8px_20px_rgba(0,0,0,0.06)] max-w-fit mx-auto justify-between pointer-events-auto gap-1 sm:gap-4">
            ...
            <div className="px-2 sm:px-8 font-black text-slate-600 text-xs sm:text-sm border-x border-slate-100 whitespace-nowrap">
              {isFullTest ? (
                <>
                  {questionData?.questionNo || (globalOffset + currentIndex + 1)} <span className="mx-0.5 text-slate-300">/</span> {globalTotal || 200}
                </>
              ) : (
                <>
            ...
            {currentIndex === data.length - 1 ? (
              isFullTest ? (
                <div className="relative group">
                  ...
                  >
                    <span>Tiếp Part 3</span> <ChevronRightIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </button>
                </div>
              ) : !isSubmitted ? (
                <button
        ...
        const footerContent = (
          <div className="relative flex-none h-14 sm:h-20 bg-white/95 backdrop-blur-md border-t border-slate-200 z-[70] flex items-center justify-between px-2 sm:px-6 pointer-events-auto shadow-[0_-10px_30px_rgba(0,0,0,0.05)] w-full" style={{ paddingRight: 'clamp(12px, 3.5vw, 72px)' }}>
