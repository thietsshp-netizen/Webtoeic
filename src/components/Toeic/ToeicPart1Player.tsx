...
import { Send, LayoutDashboard, ChevronRight, Play, Pause, Volume2, HelpCircle, CheckCircle2, XCircle, Info, Lightbulb, Flag, ChevronsLeftRight, Star, Check, X, Loader2, Edit2, Trash2, Plus, PenLine } from "lucide-react";
...
export default function ToeicPart1Player({
  ...
  const [wordFamiliesData, setWordFamiliesData] = useState<any[]>(part1WordFamiliesDataStatic);
  ...
  useEffect(() => {
    fetch("/api/word-families?part=1")
      .then((res) => res.json())
      .then((res) => {
        ...
        {!isFullTest && mounted && createPortal(
          <>
            ...
            <div
              ...
              ${isSidebarHovered ? "w-72 bg-slate-900/95 backdrop-blur-xl flex" : "w-[clamp(18px,2.2vw,32px)] bg-[#fbfcfd] border-l border-slate-100 cursor-pointer hidden lg:flex"}
            `}
                      ...
                      >
                        ...
                        {isFlaged && (
                          <div className="absolute top-1 right-1 group/flag">
                            <Flag size={10} className="shadow-sm" style={{ color: flagHex, fill: flagHex }} />
                            {flagNotes[qKey] && (
