...
export default function ToeicPart6Player({
  ...
  return (
    ...
    >
        ...
        <div className="max-w-[1600px] mx-auto bg-white rounded-xl shadow-sm border border-slate-100 flex items-center justify-between" style={{ padding: 'clamp(3px,0.5vh,8px) clamp(6px,1.2vw,14px)', gap: 'clamp(4px,0.8vw,12px)' }}>
          ...
          <div className="flex items-center shrink-0" style={{ gap: 'clamp(3px,0.6vw,8px)' }}>
            <button
              ...
              style={{ padding: 'clamp(2px,0.3vh,4px) clamp(5px,1.2vw,10px)', fontSize: 'clamp(7.5px, 2.2vw, 10.5px)' }}
            >
        ...
        <div ref={mainContainerRef} className={`flex-1 min-h-0 flex overflow-hidden relative ${isLandscape ? 'flex-row' : 'flex-col'}`} style={{ paddingBottom: isLandscape ? 'clamp(4px,0.8vh,16px)' : '0' }}>
          ...
          >
            <div className="flex-1 flex flex-col min-h-0">
              ...
              >
                <div className="mb-3 text-slate-800 italic font-medium border-l-4 border-blue-200 pl-3" style={{ fontSize: 'clamp(10px,1.5vh,15px)' }}>
                  {(() => {
                ...
                <div className="bg-white p-2" style={{ fontSize: 'clamp(11px,1.6vh,16px)', lineHeight: '1.75' }}>
                                ...
                                <div className={`group w-full flex items-center rounded-xl border text-left transition-all relative cursor-default select-text ${optClass}`} style={{ padding: 'clamp(2px,0.5vh,4px) clamp(6px,1vw,8px)', gap: 'clamp(4px,0.8vw,8px)' }}>
                                  ...
                                  <div className="flex flex-col gap-0.5 w-full">
                                    <AdminInlineEditor
                                      ...
                                      value={optText}
                                    >
                                      <span className={`font-bold ${uiState === "CORRECT" ? 'text-emerald-900' : uiState === "SELECTED" ? 'text-blue-900' : 'text-slate-900'} leading-snug`} style={{ fontSize: 'clamp(10px,1.5vh,14px)' }}>{optText}</span>
                                    </AdminInlineEditor>
                                    {isTranslationRevealed && (q.metadata as any)?.options_vn?.[opt] && (
                                      <div className="text-slate-500 font-medium italic mt-0.5 leading-tight" style={{ fontSize: 'clamp(9px,1.3vh,12px)' }}>
                                        <AdminInlineEditor
                                          target="question"
                                          id={q.id}
                                ...
                                {isCurrentRevealed && (() => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{ marginLeft: 'clamp(12px,2vh,36px)', padding: 'clamp(4px,1vh,12px)', fontSize: 'clamp(9.5px,1.4vh,13px)' }}
                                    >
      ...
      {(() => {
        const navContent = (
          <div id="toeic-navigation-container" className="flex items-center flex-nowrap whitespace-nowrap bg-white rounded-full p-0.5 sm:p-1 border border-slate-200/60 shadow-[0_8px_20px_rgba(0,0,0,0.06)] max-w-fit mx-auto justify-between pointer-events-auto gap-0.5 sm:gap-2 max-h-[clamp(26px,4.2vh,36px)]" style={{ padding: 'clamp(2px,0.4vh,5px) clamp(2px,0.4vw,6px)' }}>
            <div className="relative group shrink-0">
              <button
                ...
                className="rounded-full font-bold transition-all disabled:opacity-20 hover:bg-slate-50 text-slate-500 uppercase tracking-widest whitespace-nowrap"
                style={{ padding: 'clamp(2px,0.3vh,4px) clamp(5px,1.4vw,12px)', fontSize: 'clamp(7.5px, 2.2vw, 10.5px)' }}
              >
            ...
            <div className="font-black text-slate-600 border-x border-slate-100 whitespace-nowrap text-center shrink-0" style={{ padding: 'clamp(2px,0.3vh,4px) clamp(4px,1vw,10px)', fontSize: 'clamp(8px, 2.3vw, 11px)' }}>
              {isFullTest ? (
                <>
            ...
            {currentIndex === data.length - 1 ? (
              isFullTest ? (
                <div className="relative group shrink-0">
                  <button
                    onClick={onNextPart}
                    className="rounded-full font-bold transition-all bg-emerald-600 text-white shadow-xs hover:bg-emerald-700 active:scale-95 uppercase tracking-widest flex items-center whitespace-nowrap"
                    style={{ padding: 'clamp(2px,0.3vh,4px) clamp(5px,1.4vw,12px)', fontSize: 'clamp(7.5px, 2.2vw, 10.5px)', gap: 'clamp(2px,0.4vw,4px)' }}
                  >
                    <span className="hidden sm:inline">Tiếp Part 7</span>
                    <span className="sm:hidden">Part 7</span>
                    <ChevronRightIcon style={{ width: 'clamp(9px, 2.4vw, 12px)', height: 'clamp(9px, 2.4vw, 12px)' }} />
                  </button>
                </div>
              ) : !isSubmittedInternal && (
                <button
                  onClick={handleFinishTest}
                  disabled={isSubmitting}
                  className="rounded-full font-bold transition-all bg-indigo-600 text-white shadow-xs hover:bg-indigo-700 active:scale-95 uppercase tracking-widest whitespace-nowrap shrink-0"
                  style={{ padding: 'clamp(2px,0.3vh,4px) clamp(5px,1.4vw,12px)', fontSize: 'clamp(7.5px, 2.2vw, 10.5px)' }}
                >
              ...
              <div className="relative group shrink-0">
                <button
                  ...
                  className="rounded-full font-bold transition-all bg-indigo-600 text-white shadow-xs hover:bg-indigo-700 active:scale-95 uppercase tracking-widest whitespace-nowrap"
                  style={{ padding: 'clamp(2px,0.3vh,4px) clamp(5px,1.4vw,12px)', fontSize: 'clamp(7.5px, 2.2vw, 10.5px)' }}
                >
                  Tiếp
                </button>
